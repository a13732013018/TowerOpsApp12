...
public class WorkOrderAdapter extends RecyclerView.Adapter<WorkOrderAdapter.VH> {
    ...
    public interface OnItemLongClickListener {
         ...
         * @param order     被长按的工单
         */
        void onItemLongClick(int position, WorkOrder order);
    }
    ...
    public enum SortMode {
        ...
        ACCEPTOR_DESC,       // 接单人 Z→A
        ACCEPTOR_ASC,         // 接单人 A→Z
    ...
    private void applySort() {
        android.util.Log.d("WorkOrderAdapter", "applySort: mode=" + sortMode + ", dataSize=" + data.size());
        switch (sortMode) {
            case BILL_TIME_DESC:
                // 工单历时（timeDiff2）从大到小
            ...
            case ALERT_TIME_DESC:
                // 告警时间（实际是createTime派单时间）：最新的排最前（降序）
                Collections.sort(data, (a, b) -> {
                    long tsA = parseAlertTime(a.createTime);
                    long tsB = parseAlertTime(b.createTime);
                    if (tsA == tsB) return 0;
                    if (tsA == 0) return 1;
                    if (tsB == 0) return -1;
                    return Long.compare(tsB, tsA); // 降序
                });
                break;
            case ALERT_TIME_ASC:
                // 告警时间（实际是createTime派单时间）：最旧的排最前（升序）
                Collections.sort(data, (a, b) -> {
                    long tsA = parseAlertTime(a.createTime);
                    long tsB = parseAlertTime(b.createTime);
                    if (tsA == tsB) return 0;
                    if (tsA == 0) return 1;
                    if (tsB == 0) return -1;
                    return Long.compare(tsA, tsB); // 升序
                });
                break;
            case ACCEPTOR_DESC:
                // 接单人 Z→A（未接单排末尾）
                Collections.sort(data, (a, b) -> {
                    String sa = a.acceptOperator == null ? "" : a.acceptOperator;
                    String sb = b.acceptOperator == null ? "" : b.acceptOperator;
            ...
            case ACCEPTOR_ASC:
                ...
                Collections.sort(data, (a, b) -> {
                    ...
                    if (sa.isEmpty()) return 1;
                    if (sb.isEmpty()) return -1;
                    return sa.compareToIgnoreCase(sb); // 升序
                });
                break;
    ...
    /** 解析告警时间为毫秒时间戳，失败返回0 */
    private static long parseAlertTime(String time) {
        if (time == null || time.trim().isEmpty()) return 0;
        String s = time.trim().replace("/", "-").replace("T", " ");
        ...
        for (String fmt : patterns) {
            try {
                ...
                Date d = sdf.parse(s.substring(0, Math.min(s.length(), fmt.length())));
    ...
    public void onBindViewHolder(@NonNull VH h, int pos) {
        ...
        h.itemView.setOnLongClickListener(v -> {
            if (longClickListener != null) {
                longClickListener.onItemLongClick(h.getAdapterPosition(), finalWo);
            }
